...
func _enter_tree()-> void:
	add_tool_menu_item("Path Editor", self, "clickedButton")
	#button = Button.new()
	#button.set_text("Create or Edit Curve2D")
...
func clickedButton(ud):
	...
	ProjectSettings.set_setting("application/config/icon", "res://addons/PathEdit/icon.png")
	ProjectSettings.set_setting("application/config/name", "Edit Paths by Newold")
	ProjectSettings.save()
	var executable = OS.get_executable_path()
	var array = ["res://Addons/PathEdit/PathEdit.tscn"]
...
func _exit_tree():
	remove_tool_menu_item("Path Editor")
